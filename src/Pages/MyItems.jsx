...
const MyItems = () => {
    ...
    const handleDelete = (_id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`https://assignment-11-server-j98eqdzcq-sifats-projects-538560cb.vercel.app/foods/${_id}`
    ...
    const handleUpdate = e => {
        ...
        axios.patch(`https://assignment-11-server-j98eqdzcq-sifats-projects-538560cb.vercel.app/foods/${selectedItem._id}`, updateFoodInfo
            // {withCredentials: true}
    )
            .then(res => {
    ...
    useEffect(() => {
        if (user?.email) {
            fetch(`https://assignment-11-server-j98eqdzcq-sifats-projects-538560cb.vercel.app/my-items?email=${user.email}`, 
                // {credentials: 'include'}
            )
                .then(res => res.json())
                .then(data => {
